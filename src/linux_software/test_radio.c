...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    unsigned int bytes_transferred = 4*2048; // This might actually be 5, based on the read command being one byte?
    float time_spent = ((float)(stop_time - start_time)) / ((float)(125e6));
    float throughput = ((float)(bytes_transferred)) / ((float)(stop_time - start_time)) * ((float)(125e6)); 
    printf("You transferred %lu bytes of data in %f seconds\n",bytes_transferred,time_spent);
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 10 Aaron Roby - Custom Peripheral Demonstration\n\r");
